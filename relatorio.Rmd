...
Variáveis latente são variáveis que não são diretamente observadas, mas são inferidas através das variáveis observadas. Variáveis latente geralmente descrevem alguma característica subjacente do sistema sendo investigado.

Nas distribuições de mistura a variável latente é usada para indicar de qual distribuição a observação se origina e explorada como um instrumento para facilitar as estimações. Através de um argumento de desmarginalização, é possível assossiar uma variável aleatória $X$ de uma mistura de $k$ distribuições à uma outra variável aleatória $Z_i$ tal que:

$$ X_{i}\left|Z_{i} \sim \mathcal{N}\left(x | \theta_{z}\right), \qquad Z_{i} \sim \mathcal{M}_{k}\left(1 ; p_{1}, \ldots, p_{k}\right)\right.$$
