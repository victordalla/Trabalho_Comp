...
rmixnorm <- function(n, mean = list(0, 0), p = list(0.3, 0.7), sd = list(1, 1)) {
  if (length(mean) != length(p)) stop("Tamanhos diferem")
...
logmixnorm <- function(x, mean = list(0), p = list(1), sd = list(1, 1)) {
  ...
  sum(purrr::map_dbl(
    as_vector(x), 
    function(x) { 
      log(p[[1]]*dnorm(x, mean[[1]], 1) + p[[2]]*dnorm(x, mean[[2]], 1))
      }
    ))
}
...
grid_mat <- expand.grid(mu1,mu2)
grid_mat$mm_dens <- vector(length = length(mu1))
for (i in 1:nrow(grid_mat)){
  grid_mat$mm_dens[i] <- logmixnorm(x, c(grid_mat$Var1[i], grid_mat$Var2[i]), p)
}

ggplot(grid_mat) + stat_contour(aes(x = Var1, y = Var2, z = mm_dens))
```
