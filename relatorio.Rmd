...
Os resultados da simulação mostram que a utilização do algoritmo EM para mistura gaussiana é muito influenciado pelo valor inicial, pois dependendo dele, a estimativa do algoritmo acaba caindo em um máximo local ou ponto de cela. Logo, uma maneira de contorná-lo é iniciar com valores próximos do que se espera ser o máximo global (utilizando informações à priori e análises estatísticas), testando o algoritmo para uma variedade de valores iniciais e comparar a verossimilhança de cada estimação. 

Alem disso no caso de mistura gaussiana de duas componentes observa-se que para pontos inicias na reta identidade o algoritmo converge para um ponto de cela.
