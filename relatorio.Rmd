...
```{r}
runs <- list(run_EM(5, 3, x, p), run_EM(3, 5, x, p), run_EM(-2, -2, x, p))
...
grid.arrange(
  ...
  ggplot(grid, aes(x = mu1, y = mu2, z = log_lik)) + 
      ...
      geom_text_contour(stroke = 0.1) + 
      scale_fill_distiller(palette = "RdYlBu") + 
      geom_path(data = runs[[1]], aes(mean1, mean2), col = "lightcyan3") + 
      geom_point(data = runs[[1]], aes(mean1, mean2), col = "lightcyan4") + 
      labs(x = expression(mu[1]), y = expression(mu[2]), fill = expression(paste("l(", c(mu, p), "|", x, ")"))) + 
      theme_minimal()
)
```
